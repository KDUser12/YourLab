...
def extract_kernel_version(os_release) -> str:
    """extract_kernel_version - Extracts the kernel version

    Extended Summary:
    Extract of the kernel version since the OS release. The program searches for
    the first instance of the version in X.Y[.Z] format which it will then
    return. If the instance is not found then it will return the original version
    ...
    logging.error(f"Could not extract kernel version from: {os_release or kernel_version}")
    return os_release  # Return original version if no correspondence is found
